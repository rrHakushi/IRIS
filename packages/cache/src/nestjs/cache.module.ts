...
import { CACHE_MANAGER, CACHE_OPTIONS, CACHE_SERVICE } from './cache.constants.js';
...
export class CacheModule {
  ...
  public static forRoot(options: CacheModuleOptions = {}): DynamicModule {
    const { isGlobal = true, ...cacheOptions } = options;
    ...
    const providers: Provider[] = [
      ...
      },
      {
        ...
        useFactory: (opts: CacheOptions): CacheManager => new CacheManager(opts),
    ...
    return {
      ...
      global: isGlobal,
      providers,
      exports: [CACHE_MANAGER, CACHE_SERVICE, CacheService],
    };
  }
  ...
  private static createAsyncProviders(options: CacheModuleAsyncOptions): Provider[] {
    ...
    if (options.useClass) {
      return [
        ...
        {
          provide: CACHE_OPTIONS,
          useFactory: async (optionsFactory: CacheOptionsFactory): Promise<CacheOptions> =>
