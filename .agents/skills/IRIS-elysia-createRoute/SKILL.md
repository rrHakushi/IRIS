...
# Direct CLI scaffolding
pnpm route:create "posts/[id]/comments" -m IRIS-posts -X GET,POST --auth
```
...
|---|---|---|
| `route-path` | Target URL path (e.g. `users/[id]/profile` or `metrics`) | Prompts |
| `-m, --module <name>` | Target module in `apps/elysia/src/modules/` | `IRIS-core` |
...
```bash
pnpm route:create health -m IRIS-system -X GET
```
Generates `apps/elysia/src/modules/IRIS-system/health/route.ts` with pagination query schema.

### 2. CRUD Route with Path Parameter & Auth
```bash
pnpm route:create "articles/[id]" -m IRIS-content -X GET,PUT,DELETE --auth
```
...
```bash
pnpm route:create "admin/audit-logs" -m IRIS-account -X GET --admin -r 10
```
...
1. `generateRoutes()`: Updates `apps/elysia/src/router/routes.generated.ts` for instant Eden Treaty client types (`elysia.articles({ id: 1 }).get()`).
