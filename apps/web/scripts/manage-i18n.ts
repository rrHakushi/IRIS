  ...
  for (const file of sourceFiles) {
    ...
    lines.forEach((lineText, lineIdx) => {
      ...
      for (const [varName, ns] of Object.entries(varNamespaces)) {
        ...
        const callRegex = new RegExp(`\\b${varName}(?:\\.(?:rich|raw|has|markup))?\\(\\s*["'\`]([^"'\`]+)["'\`]`, "g");
        let cMatch;
        ...
        const stringDefaultRegex = new RegExp(`\\b${varName}(?:\\.(?:rich|raw|has|markup))?\\(\\s*["'\`]([^"'\`]+)["'\`]\\s*,\\s*(["'\`])((?:\\\\.|(?!\\2)[^\\\\])*)\\2`, "g");
        let sMatch;
        ...
        const objDefaultRegex = new RegExp(`\\b${varName}(?:\\.(?:rich|raw|has|markup))?\\(\\s*["'\`]([^"'\`]+)["'\`]\\s*,\\s*\\{[^}]*\\b(?:defaultMessage|default|fallback)\\s*:\\s*(["'\`])((?:\\\\.|(?!\\1)[^\\\\])*)\\1`, "g");
        let oMatch;
      ...
      const inlineRegex = /(?:useTranslations|getTranslations)\s*\(\s*(?:["'`]([^"'`]+)["'`])?\s*\)\s*(?:\.(?:rich|raw|has|markup))?\s*\(\s*["'`]([^"'`]+)["'`]/g;
      let inlineMatch;
...
export function runI18nManager() {
  ...
  const { references: codeReferences, defaultViolations } = extractKeysFromCode(sourceFiles);
  ...
  if (defaultViolations.length > 0) {
    ...
    for (const v of defaultViolations) {
      console.log(`   🚫 "${v.fullKey}" at ${v.file}:${v.line} has default value: "${v.defaultValue}"`);
    }
  } else {
  ...
  if (dictPlaceholderViolations.length > 0) {
    ...
    for (const p of dictPlaceholderViolations) {
      console.log(`   ⚠️ [${p.locale}.json] "${p.key}" => "${p.value}" (${p.reason})`);
    }
  } else {
    console.log("✅ All dictionary entries are fully translated with no placeholder/default values!");
  }
  ...
  const totalErrors = codeErrorCount + codeDefaultErrorCount + dictPlaceholderErrorCount + totalMissingInLocales;
