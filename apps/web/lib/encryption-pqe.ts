...
export function decryptPqeNotification(
  ...
  if (parts.length !== 3) {
    throw new Error(
      "Invalid encrypted notification format. Expected iv:authTag:cipher"
    )
  }
  ...
  const rawKemCiphertext = Uint8Array.from(atob(kemCiphertextBase64), (c) =>
    c.charCodeAt(0)
  )
...
export function encryptPqeNotification(
  ...
  const recipientPublicKey = Uint8Array.from(atob(publicKeyBase64), (c) =>
    c.charCodeAt(0)
  )
