...
export function EncryptionProvider({ children }: { children: React.ReactNode }) {
  ...
  const [hasSeparateEncryptionPassword, setHasSeparateEncryptionPassword] = useState<boolean>(false);
  ...
  const fetchKeys = useCallback(
    async (
      ...
      try {
        ...
        if (!forceServer && userId) {
          ...
          if (cached) {
            ...
            if (cached.hasSeparateEncryptionPassword !== undefined) {
              setHasSeparateEncryptionPassword(Boolean(cached.hasSeparateEncryptionPassword));
            }
        ...
        const { data, error: apiError } = await (elysia as any).users.me.encryption.get({
        ...
        console.warn("[EncryptionContext] Failed to fetch encryption vault:", err);
        return null;
      } finally {
        setIsLoading(false);
      }
  ...
  const changeEncryptionPassword = async (
    ...
    try {
      const { data, error: apiError } = await (elysia as any).users.me.encryption.post(
        {
          currentPassword,
      ...
      if (apiError || !data?.success) {
        const msg =
          typeof apiError?.value === "object" && apiError.value && "message" in apiError.value
      ...
      const msg = err instanceof Error ? err.message : "Error updating encryption password";
      setError(msg);
      return { success: false, error: msg };
    } finally {
      setIsLoading(false);
