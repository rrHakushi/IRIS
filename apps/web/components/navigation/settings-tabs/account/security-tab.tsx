  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        const msg = errorData?.message || t("incorrectCurrentPassword")
        throw new Error(msg)
      }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (isPasswordDirty) {
        ...
        if (passRes.error) {
          const errorData = passRes.error?.value as
      ...
      if (isEmailMfaDirty) {
        ...
        if (emailRes.error) {
          const errorData = emailRes.error?.value as
            { message?: string } | undefined
        ...
        toast.success(
          draftEmailMfa ? t("email2faEnabled") : t("email2faDisabled")
        )
      }
