...
export function Totp2faCard({
  ...
  // Disable state
  const [disableMethod, setDisableMethod] = useState<"code" | "password">(
    "code"
  )
  ...
  const handleVerifySetup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        const errorData = res.error?.value as { message?: string } | undefined
        const msg = errorData?.message || t("invalidVerificationCode")
        throw new Error(msg)
      }
  ...
  const handleDisableTotp = async (e?: React.FormEvent) => {
    ...
    if (
      disableMethod === "code" &&
      (!disableCode.trim() || disableCode.trim().length !== 6)
    ) {
      ...
      const res = await elysia.auth.totp.toggle.post(
        {
          ...
          password:
            disableMethod === "password" ? disablePassword.trim() : undefined,
        },
        {
          fetch: { credentials: "include" },
        }
    ...
    <>
      <div className="space-y-3.5 rounded-2xl border border-border/60 bg-muted/20 p-4 shadow-2xs sm:space-y-4 sm:p-5">
        ...
        <div className="flex flex-col justify-between gap-2 rounded-xl border border-border/50 bg-background/50 p-3 sm:flex-row sm:items-center">
          <span className="text-xs text-muted-foreground">
            {enabled ? t("totpActiveDesc") : t("totpInactiveDesc")}
          </span>
          <Button
            type="button"
            variant="outline"
            ...
            className="h-7 shrink-0 self-start rounded-lg px-3 text-xs sm:self-auto"
          >
      ...
      <Dialog isOpen={setupOpen} onOpenChange={setSetupOpen}>
        <DialogHeader>
          <div className="flex items-center gap-2 text-primary">
            ...
            <DialogTitle>
              {backupCodes
                ? t("emergencyRecoveryCodes")
                : t("setUpAuthenticatorApp")}
            </DialogTitle>
          </div>
          {backupCodes && (
            <DialogDescription>{t("emergencyRecoveryDesc")}</DialogDescription>
          )}
        </DialogHeader>
        ...
        {backupCodes ? (
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-start gap-2.5 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-300">
              ...
              <span>{t("codesShownOnceWarning")}</span>
            </div>

            <div className="grid grid-cols-2 gap-2 rounded-xl border border-border bg-muted/40 p-3 font-mono text-xs text-foreground">
              {backupCodes.map((code, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between rounded-lg border border-border/50 bg-background/80 px-2.5 py-1.5"
                >
              ...
              <>
                  ...
                  <div className="flex flex-col items-center justify-center gap-3 py-2">
                    ...
                    {secretKey && (
                      <div className="flex max-w-full items-center gap-2">
                        <div className="flex items-center gap-1.5 rounded-xl border border-border bg-muted/60 px-3 py-1.5 font-mono text-xs">
                          <IconKey className="size-3.5 shrink-0 text-muted-foreground" />
                          <span
                            className={
                              showSecret
                                ? "font-semibold select-all"
                                : "tracking-widest text-muted-foreground select-none"
                            }
                          >
                        ...
                        {showSecret && (
                          <Button
                            ...
                            className="size-8 shrink-0 animate-in rounded-xl fade-in-50"
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="123456"
                    className="h-10 rounded-xl text-center font-mono text-base tracking-widest"
                    autoFocus
                  />
                </div>
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    className="h-8 gap-1.5 rounded-xl px-4 text-xs font-semibold"
                  >
                    {isVerifying ? (
      ...
      <Dialog isOpen={disableOpen} onOpenChange={setDisableOpen}>
        <DialogHeader>
          ...
          <DialogDescription>{t("disableAuthenticatorDesc")}</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleDisableTotp} className="space-y-4 pt-1">
          ...
          <div className="grid grid-cols-2 gap-1.5 rounded-xl border border-border/60 bg-muted/60 p-1 text-xs">
            <button
              type="button"
              onClick={() => {
                setDisableMethod("code")
              ...
              className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 font-medium transition-all ${
                disableMethod === "code"
          ...
          {disableMethod === "code" ? (
            <div className="animate-in space-y-1.5 duration-150 fade-in-50">
              <label className="text-xs font-medium text-foreground">
                {t("sixDigitCode")}
              </label>
              <Input
                ...
                placeholder="123456"
                className="h-10 rounded-xl bg-background text-center font-mono text-base tracking-widest"
                autoFocus
              />
            </div>
          ) : (
            <div className="animate-in space-y-1.5 duration-150 fade-in-50">
              ...
              <div className="relative">
                <Input
                  ...
                  className="h-10 rounded-xl bg-background pe-10 text-xs"
                  autoFocus
                />
                <button
                  type="button"
                  ...
                  className="absolute end-3 top-1/2 -translate-y-1/2 cursor-pointer p-1 text-muted-foreground hover:text-foreground"
                  aria-label={
                    showDisablePassword ? t("hidePassword") : t("showPassword")
                  }
                >
                  {showDisablePassword ? (
                    <IconEyeOff className="size-3.5" />
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="h-8 cursor-pointer gap-1.5 rounded-xl px-4 text-xs font-semibold"
            >
