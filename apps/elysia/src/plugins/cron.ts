...
function parseDurationToMs(pattern: string): number | null {
  ...
  const match = clean.match(
    /^(?:every\s+)?(\d+)\s*(s|sec|seconds?|m|min|minutes?|h|hours?|d|days?)$/
  )
...
export function getNextCronDate(
  ...
  if (parts.length !== 5 && parts.length !== 6) {
    throw new Error(
      `Invalid cron pattern: "${pattern}". Expected 5 or 6 fields.`
    )
  }
  ...
  const allowedMon = parseCronField(monField, 1, 12, MONTH_ALIASES)
  const allowedDow = parseCronField(dowField, 0, 7, DOW_ALIASES)
...
export class CronManager {
  ...
  schedule(
    ...
    if (typeof patternOrConfig === "string") {
      if (!run)
        throw new Error("A run callback is required when pattern is a string.")
...
export function cron<const Configs extends readonly CronConfig[]>(
  ...
  {
    decorator: {}
    store: { cron: { [K in Configs[number]["name"]]: CronJob } }
    derive: {}
  }
>
...
export function cron(
  ...
  return new Elysia({ name: `cron-${nameSuffix}` }).state("cron", cronStore)
}
