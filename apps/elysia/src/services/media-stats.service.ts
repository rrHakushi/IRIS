    ...
    return cache.getOrSet(
      ...
      async () => {
        ...
        for (const e of filteredEntries) {
          ...
          if (totalEps <= 1) {
            lengthBuckets["Movie / Special (1 ep)"] =
              (lengthBuckets["Movie / Special (1 ep)"] ?? 0) + 1
          } else if (totalEps <= 13) {
            lengthBuckets["Short (2–13 eps)"] =
              (lengthBuckets["Short (2–13 eps)"] ?? 0) + 1
          } else if (totalEps <= 26) {
            lengthBuckets["Standard (14–26 eps)"] =
              (lengthBuckets["Standard (14–26 eps)"] ?? 0) + 1
          } else if (totalEps <= 52) {
            lengthBuckets["Long (27–52 eps)"] =
              (lengthBuckets["Long (27–52 eps)"] ?? 0) + 1
          } else {
            lengthBuckets["Massive (53+ eps)"] =
              (lengthBuckets["Massive (53+ eps)"] ?? 0) + 1
          }
          ...
          dayOfWeekMap[entryDate.getUTCDay()] =
            (dayOfWeekMap[entryDate.getUTCDay()] ?? 0) + 1
        ...
        const releaseYearGraph: YearGraphItem[] = Object.keys(releaseYearMap)
          ...
          .sort((a, b) => a - b)
          .map((year) => {
            const node = releaseYearMap[year] ?? {
              titles: 0,
              scores: [],
              minutes: 0,
            }
        ...
        const activityYearGraph: YearGraphItem[] = Object.keys(activityYearMap)
          ...
          .sort((a, b) => a - b)
          .map((year) => {
            const node = activityYearMap[year] ?? {
              titles: 0,
              scores: [],
              minutes: 0,
            }
        ...
        const { weeklyActivity, dayOfWeekActivity } =
          buildWeeklyAndDailyActivity(weeklyActivityMap, dayOfWeekMap)
    ...
    return cache.getOrSet(
      ...
      async () => {
          ...
          include: {
            manga: {
              select: {
                ...
                genres: { select: { name: true } },
                staff: {
                  select: { person: { select: { namePrimary: true } } },
                },
              },
            },
          },
        ...
        for (const e of filteredEntries) {
          ...
          if (totalChapters < 10) {
            lengthBuckets["One-Shot / Short (<10 ch)"] =
              (lengthBuckets["One-Shot / Short (<10 ch)"] ?? 0) + 1
          } else if (totalChapters <= 50) {
            lengthBuckets["Medium (10–50 ch)"] =
              (lengthBuckets["Medium (10–50 ch)"] ?? 0) + 1
          } else if (totalChapters <= 100) {
            lengthBuckets["Long (51–100 ch)"] =
              (lengthBuckets["Long (51–100 ch)"] ?? 0) + 1
          } else {
            lengthBuckets["Epic (100+ ch)"] =
              (lengthBuckets["Epic (100+ ch)"] ?? 0) + 1
          }
    ...
    return cache.getOrSet(
      ...
      async () => {
        ...
        for (const e of filtered) {
          ...
          const runtime =
            e.movie.runtime && e.movie.runtime > 0 ? e.movie.runtime : 105
          const entryMinutes =
            (e.status === "COMPLETED" ? 1 : 0) * runtime * (e.rewatched + 1)
          ...
          if (runtime < 90)
            lengthBuckets["Under 90m"] = (lengthBuckets["Under 90m"] ?? 0) + 1
          else if (runtime <= 120)
            lengthBuckets["Standard (90–120m)"] =
              (lengthBuckets["Standard (90–120m)"] ?? 0) + 1
          else if (runtime <= 150)
            lengthBuckets["Long (120–150m)"] =
              (lengthBuckets["Long (120–150m)"] ?? 0) + 1
          else
            lengthBuckets["Epic (150m+)"] =
              (lengthBuckets["Epic (150m+)"] ?? 0) + 1
        ...
        return {
          ...
          formatDistribution: [
            { name: "MOVIE", count: totalCount, percentage: 100 },
          ],
          statusDistribution: buildDistribution(statusCounts, totalCount),
          countryDistribution: buildDistribution(countryCounts, totalCount),
          ...
          topCreators: Object.entries(studioMap)
            ...
            .sort((a, b) => b.count - a.count)
            .slice(0, 30),
          monthlyActivity: Object.entries(monthlyActivityMap).map(
            ([k, count]) => {
      ...
      async () => {
        ...
        return {
          ...
          overview: {
            ...
            daysConsumed: Math.round((totalMinutes / 1440) * 10) / 10,
            daysPlanned:
              Math.round(((planningCount * 45 * 10) / 1440) * 10) / 10,
            meanScore: mean,
            standardDeviation: stdDev,
            scoredCount: scores.length,
          },
          ...
          lengthDistribution: [
            {
              label: "Mini-Series (1–6 eps)",
              count: filtered.filter(
                (i) => (i.tv.episodeCount || i.progress) <= 6
              ).length,
              description: "1-6 episodes",
            },
            {
              label: "Standard Season (7–13 eps)",
              count: filtered.filter(
                (i) =>
                  (i.tv.episodeCount || i.progress) > 6 &&
                  (i.tv.episodeCount || i.progress) <= 13
              ).length,
              description: "7-13 episodes",
            },
            {
              label: "Full Season (14–24 eps)",
              count: filtered.filter(
                (i) =>
                  (i.tv.episodeCount || i.progress) > 13 &&
                  (i.tv.episodeCount || i.progress) <= 24
              ).length,
              description: "14-24 episodes",
            },
            {
              label: "Multi-Season (25+ eps)",
              count: filtered.filter(
                (i) => (i.tv.episodeCount || i.progress) > 24
              ).length,
              description: "25+ episodes",
            },
          ],
          formatDistribution: [
            { name: "SERIES", count: totalCount, percentage: 100 },
          ],
          statusDistribution: buildDistribution(statusCounts, totalCount),
          countryDistribution: buildDistribution(countryCounts, totalCount),
          releaseYearGraph: Object.keys(releaseYearMap)
          ...
          weeklyActivity: buildWeeklyAndDailyActivity(
            weeklyActivityMap,
            dayOfWeekMap
          ).weeklyActivity,
          dayOfWeekActivity: buildWeeklyAndDailyActivity(
            weeklyActivityMap,
            dayOfWeekMap
          ).dayOfWeekActivity,
        }
      ...
      async () => {
        ...
        return {
          ...
          overview: {
            ...
            daysConsumed:
              Math.round(((totalPlaytimeHours * 60) / 1440) * 10) / 10,
            daysPlanned:
              Math.round(((planningCount * 25 * 60) / 1440) * 10) / 10,
            meanScore: mean,
            standardDeviation: stdDev,
            scoredCount: scores.length,
          },
          ...
          lengthDistribution: [
            {
              label: "Quick (<10 hrs)",
              count: filtered.filter((i) => (i.progress || 0) < 10).length,
              description: "<10 hours",
            },
            {
              label: "Regular (10–30 hrs)",
              count: filtered.filter(
                (i) => (i.progress || 0) >= 10 && (i.progress || 0) <= 30
              ).length,
              description: "10-30 hours",
            },
            {
              label: "Deep (30–80 hrs)",
              count: filtered.filter(
                (i) => (i.progress || 0) > 30 && (i.progress || 0) <= 80
              ).length,
              description: "30-80 hours",
            },
            {
              label: "Massive (80+ hrs)",
              count: filtered.filter((i) => (i.progress || 0) > 80).length,
    ...
    return cache.getOrSet(
      ...
      async () => {
        ...
        for (const e of filtered) {
          ...
          const pages =
            e.progressPages ||
            (e.status === "COMPLETED" ? e.book.pageCount || 300 : 0)
        ...
        return {
          ...
          overview: {
            ...
            daysConsumed: Math.round((totalMinutes / 1440) * 10) / 10,
            daysPlanned:
              Math.round(((planningCount * 300 * 1.2) / 1440) * 10) / 10,
            meanScore: mean,
            standardDeviation: stdDev,
            scoredCount: scores.length,
          },
          ...
          lengthDistribution: [
            {
              label: "Novella (<200 pages)",
              count: filtered.filter((i) => (i.book.pageCount || 250) < 200)
                .length,
              description: "<200 pages",
            },
            {
              label: "Regular (200–400 pages)",
              count: filtered.filter(
                (i) =>
                  (i.book.pageCount || 250) >= 200 &&
                  (i.book.pageCount || 250) <= 400
              ).length,
              description: "200-400 pages",
            },
            {
              label: "Thick (400–700 pages)",
              count: filtered.filter(
                (i) =>
                  (i.book.pageCount || 250) > 400 &&
                  (i.book.pageCount || 250) <= 700
              ).length,
              description: "400-700 pages",
            },
            {
              label: "Epic (700+ pages)",
              count: filtered.filter((i) => (i.book.pageCount || 250) > 700)
                .length,
              description: "700+ pages",
            },
          ],
          formatDistribution: [
            { name: "BOOK", count: totalCount, percentage: 100 },
          ],
          statusDistribution: buildDistribution(statusCounts, totalCount),
          countryDistribution: [],
          releaseYearGraph: Object.keys(releaseYearMap)
    ...
    return cache.getOrSet(
      ...
      async () => {
        ...
        return {
          ...
          overview: {
            totalCount,
            completedCount: filtered.filter((i) => i.status === "COMPLETED")
              .length,
            currentCount: filtered.filter((i) => i.status === "LISTENING")
              .length,
            planningCount: filtered.filter((i) => i.status === "PLANNING")
              .length,
            onHoldCount: 0,
    ...
    return cache.getOrSet(
      ...
      async () => {
        ...
        for (const { stats } of allStats) {
          ...
          scoreWeightedSum +=
            stats.overview.meanScore * stats.overview.scoredCount
        }
        ...
        const combinedCreatorMap: Record<
          string,
        ...
        for (const { stats } of allStats) {
          ...
          if (stats.dayOfWeekActivity) {
            for (const d of stats.dayOfWeekActivity) {
              combinedDayOfWeekMap[d.day] =
                (combinedDayOfWeekMap[d.day] ?? 0) + d.count
            }
          }
        ...
        const { weeklyActivity, dayOfWeekActivity } =
          buildWeeklyAndDailyActivity(combinedWeeklyMap, combinedDayOfWeekMap)
