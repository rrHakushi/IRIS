...
export class MediaQueueService {
  ...
  public static logStatus(): void {
    ...
    logger.service("media-queue", "distributed media metadata queue & sync pipeline");

    const missingRequired: Array<{ provider: string; envVar: string; reason: string }> = [];
    const missingOptional: Array<{ provider: string; envVar: string; fallback: string }> = [];
    ...
    if (!igdbId || !igdbSecret) {
      const missing = [!igdbId && "IGDB_CLIENT_ID", !igdbSecret && "IGDB_CLIENT_SECRET"].filter(Boolean).join(", ");
      missingRequired.push({
        provider: "IGDB",
        envVar: missing,
    ...
    for (const item of missingOptional) {
      logger.service.providerOptionalEnv(item.provider, item.envVar, item.fallback);
    }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const processingKeys = await this.getRedisSet(`${REDIS_KEY_PREFIX}:processing`);
      ...
      for (const jobId of allJobIds) {
        const jobData = await cache.get<MediaJob>(`${REDIS_KEY_PREFIX}:job:${jobId}`);
        if (jobData && jobData.status !== "COMPLETED" && jobData.status !== "CANCELLED") {
  ...
  private async checkExistingFreshness(
    ...
    switch (type) {
      ...
      case "TV": {
        if (isNaN(extIdNum)) return false;
        const record = await prisma.tv.findUnique({ where: { tvDBId: extIdNum } });
        return !mediaDbSyncer.isRecordStale(record, "TV");
      }
      case "MOVIE": {
        if (isNaN(extIdNum)) return false;
        const record = await prisma.movie.findUnique({ where: { tvDBId: extIdNum } });
        return !mediaDbSyncer.isRecordStale(record, "MOVIE");
      }
      case "BOOK": {
        const idStr = String(externalId);
        const record = await prisma.book.findFirst({
          where: { OR: [{ googleBookId: idStr }, { isbn13: idStr }, { isbn10: idStr }] },
        });
        return !mediaDbSyncer.isRecordStale(record, "BOOK");
      }
      case "GAME": {
        if (isNaN(extIdNum)) return false;
        const record = await prisma.game.findUnique({ where: { igdbId: extIdNum } });
        return !mediaDbSyncer.isRecordStale(record, "GAME");
      }
      case "MUSIC": {
        const idStr = String(externalId);
        const record = await prisma.music.findUnique({ where: { musicBrainzId: idStr } });
  ...
  private async executeJob(job: MediaJob): Promise<void> {
    ...
    switch (job.type) {
      case "ANIME": {
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 [1/3] Querying AniList GraphQL API for Anime #${anilistId}...`);
        const alData = await this.anilist.fetchAnime(anilistId);
        summaryText = alData.title.userPreferred || alData.title.english || alData.title.romaji || "";
        ...
        let malData: import("./providers/mal.provider.js").MalAnimePayload | null = null;
        let malEpisodes: import("./providers/mal.provider.js").MalScrapedEpisode[] = [];
        const malId = alData.idMal;
        if (malId) {
          try {
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 [2/3] Querying Official MyAnimeList API v2 for MAL ID #${malId}...`);
            ...
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} ⚠️ [2/3] MAL supplement warning: ${malErr.message}`);
          }
        ...
        if (mappedIds.anidbId || mappedIds.tvdbId || mappedIds.bangumiId || mappedIds.kitsuId) {
          const parts: string[] = [];
          ...
          logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 🔗 [2/3] Mapped IDs: ${parts.join(", ")}`);
        }
        ...
        if (mappedIds.tvdbId) {
          try {
            ...
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} ⚠️ [2/3] TheTVDB images warning: ${tvdbErr.message}`);
          }
        }
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 💾 [3/3] Upserting Anime to database using Local IDs...`);
      ...
      case "MANGA": {
        const anilistId = Number(job.externalId);
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 [1/3] Querying AniList GraphQL API for Manga #${anilistId}...`);
        ...
        let malData: import("./providers/mal.provider.js").MalMangaPayload | null = null;
        const malId = alData.idMal;
        if (malId) {
          try {
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 [2/3] Querying Official MyAnimeList API v2 for Manga MAL ID #${malId}...`);
        ...
        if (mappedMangaIds.mangaUpdatesId || mappedMangaIds.kitsuId || mappedMangaIds.bangumiId) {
          const parts: string[] = [];
          if (mappedMangaIds.mangaUpdatesId) parts.push(`MangaUpdates=#${mappedMangaIds.mangaUpdatesId}`);
          if (mappedMangaIds.kitsuId) parts.push(`Kitsu=#${mappedMangaIds.kitsuId}`);
          if (mappedMangaIds.bangumiId) parts.push(`Bangumi=#${mappedMangaIds.bangumiId}`);
          logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 🔗 [2/3] Mapped Manga IDs: ${parts.join(", ")}`);
        }

        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 💾 [3/3] Upserting Manga to database using Local IDs...`);
        const result = await mediaDbSyncer.upsertManga(alData, malData, mappedMangaIds);
        localId = result.id;
        relationsToCrawl = result.discoveredRelations;
      ...
      case "TV": {
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 Querying TheTVDB API v4 for Series #${tvdbId}...`);
        ...
        if (series.remoteIds) {
          for (const r of series.remoteIds) {
            if (r.sourceName?.toUpperCase() === "IMDB" || r.type === 2) imdbId = r.id;
        ...
        try {
          simklData = await this.simkl.lookupTv({ imdbId, tmdbId, tvdbId, title: series.name });
          if (simklData) {
            const parts: string[] = [];
            if (simklData.simklId) parts.push(`Simkl=#${simklData.simklId}`);
            if (simklData.imdbRating) parts.push(`IMDb=${simklData.imdbRating}★ (${simklData.imdbVotes || 0} votes)`);
        ...
        try {
          engTranslation = await this.tvdb.fetchTvSeriesTranslation(tvdbId, "eng");
      ...
      case "MOVIE": {
        const tvdbId = Number(job.externalId);
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 Querying TheTVDB API v4 for Movie #${tvdbId}...`);
        ...
        try {
          simklData = await this.simkl.lookupMovie({ imdbId, tmdbId, tvdbId, title: movie.name });
          if (simklData) {
            const parts: string[] = [];
            if (simklData.simklId) parts.push(`Simkl=#${simklData.simklId}`);
            if (simklData.imdbRating) parts.push(`IMDb=${simklData.imdbRating}★ (${simklData.imdbVotes || 0} votes)`);
      ...
      case "BOOK": {
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 Querying Google Books API for "${volumeId}"...`);
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 💾 Upserting Book to database...`);
      ...
      case "GAME": {
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 Querying IGDB API v4 for Game #${igdbId}...`);
        ...
        if (!steamAppId && gameData.websites) {
          const steamWeb = gameData.websites.find((w) => w.url.includes("store.steampowered.com/app/"));
          if (steamWeb) {
        ...
        if (steamAppId) {
          try {
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 🎮 Querying Steam Store API for App #${steamAppId}...`);
            ...
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} ⚠️ Steam details warning: ${steamErr.message}`);
          }

          try {
            deckData = await this.steam.fetchDeckCompatibility(steamAppId);
            if (deckData) {
              const catNames: Record<number, string> = { 1: "Unsupported", 2: "Playable", 3: "Verified" };
        ...
        const result = await mediaDbSyncer.upsertGame(gameData, steamData, deckData);
        localId = result.id;
        break;
      }
      ...
      case "MUSIC": {
        ...
        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 Querying MusicBrainz WS/2 for Recording "${mbid}"...`);
        ...
        let lyrics: import("./providers/lrclib.provider.js").LrcLibLyricsPayload | null = null;
        if (track && artist) {
          logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📡 Querying LRCLIB for lyrics ("${track}" - "${artist}")...`);
          lyrics = await this.lrclib.fetchLyrics(track, artist, album, duration);
          if (lyrics) {
            logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 📥 LRCLIB lyrics received (${lyrics.plainLyrics ? `${lyrics.plainLyrics.length} chars` : "synced only"})`);
          }
        }

        logQueue(`${c.magenta(c.bold("[MediaQueue]"))} 💾 Upserting Music track to database...`);
  ...
  private async resolveCharacterDescriptionLinks(characterIds: number[]): Promise<void> {
    ...
    for (const charId of characterIds) {
      try {
        ...
        if (!char?.description || !char.description.includes("anilist.co/character/")) {
          continue;
        }
