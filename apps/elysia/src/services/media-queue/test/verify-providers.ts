...
async function runVerification() {
  ...
  try {
    ...
    console.log(`  ✓ AniList Anime: "${anime.title.userPreferred || anime.title.romaji}" (ID: ${anime.id})`);
    console.log(`    - Episodes: ${anime.episodes}, Relations: ${anime.relations?.edges?.length || 0}`);
    console.log(`    - Characters: ${anime.characters?.edges?.length || 0}, Staff: ${anime.staff?.edges?.length || 0}`);
  ...
  try {
    ...
    console.log(`  ✓ MAL Anime: "${malAnime.title}" (Mean: ${malAnime.mean}, Rating: ${malAnime.rating})`);
    console.log(`    - OP Themes: ${malAnime.opening_themes?.length || 0}, ED Themes: ${malAnime.ending_themes?.length || 0}`);
  } catch (err: any) {
  ...
  try {
    ...
    console.log(`  ✓ TVDB TV Series: "${tvSeries.name}" (Status: ${tvSeries.status?.name})`);
  ...
  const isFinishedStale = mediaDbSyncer.isRecordStale(testFinishedRecord, "ANIME");
  console.log(`  - Finished anime updated 30 days ago is stale? ${isFinishedStale} (Expected: false)`);
