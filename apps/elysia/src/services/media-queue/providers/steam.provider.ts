...
export class SteamProvider {
  ...
  async fetchAppDetails(appId: number): Promise<SteamAppDetailsPayload | null> {
    ...
    const json = (await res.json()) as Record<string, { success: boolean; data?: SteamAppDetailsPayload }>;
   ...
   */
  async fetchDeckCompatibility(appId: number): Promise<SteamDeckCompatibilityReport | null> {
    await this.waitForRateLimit();
    ...
    const json = (await res.json()) as { success: number; results?: SteamDeckCompatibilityReport };
