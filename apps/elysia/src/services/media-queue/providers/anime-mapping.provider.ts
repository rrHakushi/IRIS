...
export class AnimeMappingProvider {
    ...
    try {
      ...
      if (Array.isArray(bangumiItems)) {
        for (const item of bangumiItems) {
          ...
          if (alSite?.id) this.bangumiAnilistMap.set(parseInt(alSite.id, 10), bgmId);

          const malSite = item.sites?.find((s: any) => s.site === "mal");
          if (malSite?.id) this.bangumiMalMap.set(parseInt(malSite.id, 10), bgmId);
        }
      }
    } catch (err: any) {
      console.warn(`[AnimeMappingProvider] Warning: failed to load remote mappings: ${err.message}`);
    }
  }
   ...
   */
  async lookup(identifiers: { anilistId?: number; malId?: number }): Promise<AnimeMappingEntry> {
    await this.ensureLoaded();

    const entry =
      (identifiers.anilistId ? this.anilistMap.get(identifiers.anilistId) : null) ||
      (identifiers.malId ? this.malMap.get(identifiers.malId) : null);

    const bangumiId =
      (identifiers.anilistId ? this.bangumiAnilistMap.get(identifiers.anilistId) : null) ||
      (identifiers.malId ? this.bangumiMalMap.get(identifiers.malId) : null);

    const imdbId = Array.isArray(entry?.imdb_id) ? entry.imdb_id[0] : entry?.imdb_id;
    const tmdbId =
  ...
  async lookupManga(identifiers: {
    ...
    const keyPart = identifiers.anilistId || identifiers.malId || encodeURIComponent(identifiers.title);
    ...
    const muPromise = (async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = (await res.json()) as { results?: Array<{ record?: { series_id?: number } }> };
          if (data.results && data.results.length > 0 && data.results[0]?.record?.series_id) {
