...
export class MyAnimeListProvider {
  ...
  private async fetchJson<T>(url: string): Promise<T> {
    ...
    if (!clientId) {
      throw new Error("[MyAnimeListProvider] MAL_CLIENT_ID is not configured in .env");
    }
  ...
  async fetchAnimeEpisodes(malId: number): Promise<MalScrapedEpisode[]> {
    ...
    while (hasMore && offset <= 1000) {
      try {
        ...
        const rowRegex = /<tr class="[^"]*episode-list-data[^"]*"[\s\S]*?<\/tr>/g;
        let countInPage = 0;
        let rowMatch;
        ...
        while ((rowMatch = rowRegex.exec(html)) !== null) {
          ...
          const titlePrimary = this.decodeHtml(linkMatch[3].replace(/<[^>]+>/g, "").trim());

          const subMatch = rowHtml.match(/<span\s+class="di-ib">([\s\S]*?)<\/span>/);
          let titleSecondary: string | undefined;
          let titleNative: string | undefined;
          if (subMatch) {
            const subText = this.decodeHtml(
              subMatch[1].replace(/<[^>]+>/g, "").replace(/&nbsp;/g, " ").trim()
            );
          ...
          const airMatch = rowHtml.match(/<td\s+class="episode-aired[^"]*">([\s\S]*?)<\/td>/);
          ...
          const isFiller =
            rowHtml.includes("icon-filler") || /class="[^"]*filler[^"]*"/.test(rowHtml);
          const isRecap =
            rowHtml.includes("icon-recap") || /class="[^"]*recap[^"]*"/.test(rowHtml);
          ...
          episodes.push({
            ...
            titlePrimary,
            titleSecondary:
              titleSecondary && titleSecondary !== titlePrimary ? titleSecondary : undefined,
            titleNative,
            airDate,
            isFiller,
            isRecap,
      ...
      await Promise.all(
        batch.map(async (ep) => {
          try {
            ...
            const durMatch = epHtml.match(/<span class="fw-b">Duration:\s*<\/span>\s*([\d:]+)/i);
            ...
            if (synMatch && synMatch[1]) {
              const raw = synMatch[1].replace(/<br\s*\/?>/gi, "\n").replace(/<[^>]+>/g, "").trim();
