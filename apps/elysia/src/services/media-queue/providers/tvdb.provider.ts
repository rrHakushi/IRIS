...
export interface TvdbSeriesPayload {
  ...
  contentRatings?: Array<{ name: string; country: string; description?: string; fullname?: string }>;
  airsDays?: {
    sunday?: boolean;
    monday?: boolean;
    tuesday?: boolean;
...
export class TheTVDBProvider {
  ...
  private async getValidToken(): Promise<string> {
    ...
    if (!apiKey) {
      throw new Error("[TheTVDBProvider] THETVDB_KEY / TVDB_API_KEY is not configured in .env");
    }
    ...
    if (!res.ok) {
      ...
      throw new Error(`[TheTVDBProvider] Login failed HTTP ${res.status}: ${errText}`);
    }
  ...
  private async fetchJson<T>(endpoint: string): Promise<T> {
    ...
    if (!res.ok) {
      const errText = await res.text().catch(() => "");
      throw new Error(`[TheTVDBProvider] HTTP ${res.status} for ${endpoint}: ${errText}`);
    }
  ...
  async fetchTvSeriesTranslation(
    ...
    try {
      return await this.fetchJson<{ name?: string; overview?: string; language?: string }>(
  ...
  async fetchTvCharacters(tvdbId: number): Promise<TvdbCharacter[]> {
    try {
      return await this.fetchJson<TvdbCharacter[]>(`/series/${tvdbId}/characters`);
    } catch {
      return [];
    }
  }
