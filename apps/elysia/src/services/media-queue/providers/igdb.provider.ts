...
export class IGDBProvider {
  ...
  private async getValidToken(): Promise<string> {
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        "[IGDBProvider] IGDB_CLIENT_ID or IGDB_CLIENT_SECRET is missing in .env"
      )
    }
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `[IGDBProvider] Twitch OAuth token failed HTTP ${res.status}: ${errText}`
      )
    }
  ...
  async fetchGame(igdbId: number): Promise<IgdbGamePayload> {
    ...
    if (!res.ok) {
      const errText = await res.text().catch(() => "")
      throw new Error(
        `[IGDBProvider] Games query HTTP ${res.status}: ${errText}`
      )
    }
    ...
    if (
      ...
      game.age_ratings[0]?.rating === undefined
    ) {
   ...
   */
  async fetchAgeRatings(ids: number[]): Promise<
    Array<{
      id: number
      organization?: number
      rating_category?: number
