...
export class MediaDbSyncer {
  ...
  private async upsertTags(
    tags?: Array<string | { name: string; category?: string; description?: string }>
  ): Promise<{ id: number }[]> {
    ...
    for (const item of tags) {
      ...
      const slug = key.replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      const category = typeof item === "object" ? item.category?.trim() : undefined;
      const description = typeof item === "object" ? item.description?.trim() : undefined;
  ...
  private async upsertPerson(data: {
    ...
    if (data.anilistId) {
      const existing = await prisma.person.findUnique({ where: { anilistId: data.anilistId } });
      if (existing) {
    ...
    if (data.malId) {
      const existing = await prisma.person.findUnique({ where: { malId: data.malId } });
      if (existing) {
        if (data.language && !existing.language) {
          await prisma.person.update({
            where: { id: existing.id },
    ...
    if (data.tvDBId) {
      const existing = await prisma.person.findUnique({ where: { tvDBId: data.tvDBId } });
      if (existing) {
        if (data.language && !existing.language) {
          await prisma.person.update({
            where: { id: existing.id },
    ...
    if (data.namePrimary) {
      const existing = await prisma.person.findFirst({
        where: { namePrimary: { equals: data.namePrimary, mode: "insensitive" } },
      });
      if (existing) {
  ...
  public async upsertCharacter(data: {
    ...
    if (data.anilistId) {
      const existing = await prisma.character.findUnique({ where: { anilistId: data.anilistId } });
      if (existing) return existing;
    }
    if (data.malId) {
      const existing = await prisma.character.findUnique({ where: { malId: data.malId } });
      if (existing) return existing;
    }
    if (data.tvDBId) {
      const existing = await prisma.character.findUnique({ where: { tvDBId: data.tvDBId } });
      if (existing) return existing;
    }
        ...
        await prisma.character.update({
          ...
          data: {
            ...
            nameAlternativeSpoiler:
              existing.nameAlternativeSpoiler && existing.nameAlternativeSpoiler.length > 0
  ...
  private async upsertStudio(data: {
    ...
    if (data.anilistId) {
      const existing = await prisma.studio.findUnique({ where: { anilistId: data.anilistId } });
      if (existing) return existing;
    }
    if (data.malId) {
      const existing = await prisma.studio.findUnique({ where: { malId: data.malId } });
      if (existing) return existing;
    }
  ...
  async upsertAnime(
    ...
    tvdbImages?: string[],
    skipMap?: Map<number, import("./providers/aniskip.provider.js").EpisodeSkipTimestamps>
  ): Promise<{ id: number; discoveredRelations: DiscoveredRelation[]; characterIds: number[] }> {
    const titlePrimary =
      al.title.english || al.title.userPreferred || al.title.romaji || "Unknown Anime";
    const titleSecondary = al.title.romaji || al.title.english;
    const titleNative = al.title.native;

    const coverImage = al.coverImage?.extraLarge || al.coverImage?.large || al.coverImage?.medium;
    ...
    if (al.coverImage?.large && !anilistImages.includes(al.coverImage.large)) anilistImages.push(al.coverImage.large);
    if (al.coverImage?.medium && !anilistImages.includes(al.coverImage.medium)) anilistImages.push(al.coverImage.medium);
    if (al.bannerImage && !anilistImages.includes(al.bannerImage)) anilistImages.push(al.bannerImage);

    const malImages: string[] = [];
    if (mal?.main_picture?.large) malImages.push(mal.main_picture.large);
    if (mal?.main_picture?.medium && !malImages.includes(mal.main_picture.medium)) malImages.push(mal.main_picture.medium);
    if (mal?.pictures) {
      for (const pic of mal.pictures) {
        if (pic.large && !malImages.includes(pic.large)) malImages.push(pic.large);
        if (pic.medium && !malImages.includes(pic.medium)) malImages.push(pic.medium);
      }
    }
    ...
    const externalLinks = al.externalLinks && al.externalLinks.length > 0 ? al.externalLinks : undefined;
    ...
    const data: any = {
      ...
      averageScore: al.averageScore ? al.averageScore / 10 : mal?.mean,
      ...
      malUpdatedAt: mal?.updated_at
        ? Math.floor(new Date(mal.updated_at).getTime() / 1000)
        : (mal ? Math.floor(Date.now() / 1000) : undefined),
    };
    ...
    if (al.airingSchedule?.nodes && al.airingSchedule.nodes.length > 0) {
      ...
      if (!scheduleMap.has(1)) {
        const sortedEpisodes = [...scheduleMap.entries()].sort((a, b) => a[0] - b[0]);
        if (sortedEpisodes.length > 0) {
          ...
          const ep1Time = new Date(epTime - (epNum - 1) * 7 * 24 * 60 * 60 * 1000);
          scheduleMap.set(1, { airingAt: ep1Time });
        } else if (al.startDate?.year && al.startDate?.month && al.startDate?.day) {
          scheduleMap.set(1, {
            airingAt: new Date(Date.UTC(al.startDate.year, al.startDate.month - 1, al.startDate.day)),
          });
        }
      }
    ...
    const malEpMap = new Map<number, import("./providers/mal.provider.js").MalScrapedEpisode>();
    if (malEpisodes) {
    ...
    const maxSchedEp = scheduleMap.size > 0 ? Math.max(...Array.from(scheduleMap.keys())) : 0;
    const maxMalEp = malEpMap.size > 0 ? Math.max(...Array.from(malEpMap.keys())) : 0;
    const totalEpisodes =
      al.episodes ||
      mal?.num_episodes ||
      maxSchedEp ||
    ...
    const streamingMap = new Map<number, { title?: string; thumbnail?: string; url?: string; site?: string }>();
    if (al.streamingEpisodes) {
      for (let i = 0; i < al.streamingEpisodes.length; i++) {
        const se = al.streamingEpisodes[i]!;
        const match = se.title?.match(/Episode\s*(\d+)/i) || se.title?.match(/#(\d+)/i);
    ...
    if (totalEpisodes > 0) {
      for (let epNum = 1; epNum <= totalEpisodes; epNum++) {
        ...
        const airDate =
          malEp?.airDate ||
          sched?.airingAt ||
          (epNum === 1 && al.startDate?.year && al.startDate?.month && al.startDate?.day
            ? new Date(Date.UTC(al.startDate.year, al.startDate.month - 1, al.startDate.day))
            : undefined);

        const titlePrimary = malEp?.titlePrimary || streamInfo?.title || `Episode ${epNum}`;
        ...
        const duration = malEp?.duration || al.duration || mal?.average_episode_duration;
        ...
        const skipTimestamps = skipTimes?.rawResults ? (skipTimes.rawResults as any) : undefined;
        ...
        await prisma.animeEpisode.upsert({
          ...
          update: {
            ...
            airDate,
            streamingLinks: streamInfo ? [{ site: streamInfo.site, url: streamInfo.url }] : undefined,
            opStart,
            opEnd,
            edStart,
            edEnd,
      ...
      for (const edge of al.characters.edges) {
        ...
        if (edge.voiceActors && edge.voiceActors.length > 0) {
          for (const va of edge.voiceActors) {
            ...
            await prisma.mediaCharacter.upsert({
              where: {
    ...
    if (al.relations?.edges) {
      for (const edge of al.relations.edges) {
        ...
        const targetType: MediaJobType = edge.node.type === "MANGA" ? "MANGA" : "ANIME";
        const targetExternalId = edge.node.id;

        // Find or create local stub record for target
        let localTargetId: number;
        if (targetType === "ANIME") {
          let targetAnime = await prisma.anime.findUnique({ where: { anilistId: targetExternalId } });
          if (!targetAnime) {
          ...
          let targetManga = await prisma.manga.findUnique({ where: { anilistId: targetExternalId } });
          if (!targetManga) {
        ...
        await prisma.mediaRelation.upsert({
          where: {
  ...
  async upsertManga(
    ...
    const titlePrimary =
      al.title.english || al.title.userPreferred || al.title.romaji || "Unknown Manga";
    const titleSecondary = al.title.romaji || al.title.english;
    const titleNative = al.title.native;
    ...
    const data: any = {
      ...
      endDateMonth: al.endDate?.month,
      endDateDay: al.endDate?.day,
      genres: existing ? { set: mangaGenreRecords } : { connect: mangaGenreRecords },
    ...
    if (al.characters?.edges) {
      for (const edge of al.characters.edges) {
        ...
        const role = edge.role === "MAIN" ? "MAIN" : edge.role === "SUPPORTING" ? "SUPPORTING" : "BACKGROUND";
        const existingMC = await prisma.mediaCharacter.findFirst({
          where: {
            mediaType: "MANGA",
    ...
    if (al.relations?.edges) {
      for (const edge of al.relations.edges) {
        ...
        const targetType: MediaJobType = edge.node.type === "ANIME" ? "ANIME" : "MANGA";
        const targetExternalId = edge.node.id;
  ...
  async upsertTv(
    ...
    const existing = await prisma.tv.findUnique({ where: { tvDBId: series.id } });
    ...
    const titleSecondary =
      engTranslation?.name && engTranslation.name !== series.name
        ? series.name
        : (simklData?.title && simklData.title !== titlePrimary ? simklData.title : undefined);

    const titleNative =
      series.originalLanguage &&
      ["kor", "jpn", "zho", "zhtw", "ko", "ja", "zh"].includes(series.originalLanguage.toLowerCase())
        ? series.name
        : undefined;

    // Description resolution (English overview is ALWAYS primary)
    const description = engTranslation?.overview?.trim() || simklData?.overview || series.overview;
    ...
    if (series.remoteIds) {
      for (const r of series.remoteIds) {
        if (r.sourceName?.toUpperCase() === "IMDB" || r.type === 2) {
          imdbId = r.id;
        } else if (r.sourceName?.toLowerCase().includes("themoviedb") || r.type === 12 || r.type === 10) {
    ...
    const countryOfOrigin =
      ...
      simklData?.country ||
      (series.originalLanguage ? series.originalLanguage.toUpperCase() : undefined);
    ...
    const sources: Record<string, unknown> = {
      thetvdb: {
        id: series.id,
        slug: series.slug,
        url: series.slug ? `https://thetvdb.com/series/${series.slug}` : `https://thetvdb.com/dereferrer/series/${series.id}`,
      },
    };
    if (imdbId) {
    ...
    if (tmdbId) {
      sources.tmdb = { id: tmdbId, url: `https://www.themoviedb.org/tv/${tmdbId}` };
    }
    if (simklData?.simklId) {
      sources.simkl = {
        id: simklData.simklId,
        slug: simklData.slug,
        url: simklData.slug ? `https://simkl.com/tv/${simklData.simklId}/${simklData.slug}` : `https://simkl.com/tv/${simklData.simklId}`,
      };
    }
    ...
    const rawTvGenres = series.genres?.map((g) => g.name) || simklData?.genres || [];
    const tvGenreRecords = await this.upsertGenres(rawTvGenres);

    // Tags & Subgenres (from TVDB tags and Simkl genres -> Tag table)
    const rawTags: Array<{ name: string; category?: string; description?: string }> = [];
    ...
    const avgRuntime =
      ...
      (validEpisodes.length > 0
        ? Math.round(
            validEpisodes.reduce((acc, e) => acc + (e.runtime || 0), 0) / validEpisodes.length
          )
        : undefined);
    ...
    if (backgroundArtworks.length > 0) {
      const chosen = backgroundArtworks[Math.floor(Math.random() * backgroundArtworks.length)];
    ...
    const data: any = {
      ...
      titleNative,
      description,
      coverImage: series.image ? normalizeTvdbImageUrl(series.image) : simklData?.poster,
      bannerImage,
      images,
      sources,
      ...
      networks: series.networks?.map((n) => n.name) || (simklData?.network ? [simklData.network] : []),
      status: this.mapTvStatus(series.status?.name),
      averageScore: series.score ? series.score : simklData?.simklRating,
      ...
      imdbUpdatedAt: simklData?.imdbRating ? Math.floor(Date.now() / 1000) : undefined,
    };
    ...
    for (const sNum of seasonNumbers) {
      ...
      const seasonEpisodes = validEpisodes.filter((e) => e.seasonNumber === sNum);
      ...
      const season = await prisma.tvSeason.upsert({
        ...
        update: {
          ...
          posterImage: seasonMeta?.image ? normalizeTvdbImageUrl(seasonMeta.image) : undefined,
          airDateYear,
          airDateMonth,
          airDateDay,
    ...
    for (const c of allCastAndStaff) {
      ...
      if (isActor && localCharId) {
        await prisma.mediaCharacter.upsert({
          ...
          update: { tvId: localTvId, role: c.isFeatured ? "MAIN" : "SUPPORTING" },
          create: {
            mediaType: "TV",
            mediaId: localTvId,
            tvId: localTvId,
    ...
    if (Array.isArray(series.companies)) {
      ...
      for (const list of [compsObj.studio, compsObj.production, compsObj.network, compsObj.distributor]) {
  ...
  async upsertMovie(
    ...
    tvdbImages?: string[],
    simklData?: import("./providers/simkl.provider.js").SimklMoviePayload | null,
    engTranslation?: { name?: string; overview?: string } | null
  ): Promise<{ id: number }> {
    const existing = await prisma.movie.findUnique({ where: { tvDBId: movie.id } });
    ...
    const titleNative =
      ...
      ["kor", "jpn", "zho", "zhtw", "ko", "ja", "zh"].includes(movie.originalLanguage.toLowerCase())
        ? movie.name
        : undefined;

    // Description resolution (English overview is ALWAYS primary)
    const description = engTranslation?.overview?.trim() || simklData?.overview || movie.overview;
    ...
    const rawReleaseDate =
      ...
      (movie.releases && movie.releases.length > 0 ? movie.releases[0]?.date : undefined) ||
    ...
    if (movie.remoteIds) {
      for (const r of movie.remoteIds) {
        if (r.sourceName?.toUpperCase() === "IMDB" || r.type === 2) {
          imdbId = r.id;
        } else if (r.sourceName?.toLowerCase().includes("themoviedb") || r.type === 10) {
          const parsed = parseInt(r.id, 10);
          if (!isNaN(parsed)) tmdbId = parsed;
        }
      }
    ...
    const countryOfOrigin =
      ...
      simklData?.country ||
      (movie.originalLanguage ? movie.originalLanguage.toUpperCase() : undefined);
    ...
    const rawGenres = movie.genres?.map((g) => g.name) || simklData?.genres || [];
    const genreRecords = await this.upsertGenres(rawGenres);
    ...
    const data: any = {
      ...
      titleNative,
      description,
      coverImage: movie.image ? normalizeTvdbImageUrl(movie.image) : simklData?.poster,
      images,
      sources,
      ...
      averageScore: movie.score ? movie.score : simklData?.simklRating,
      imdbRating: simklData?.imdbRating,
      imdbVotes: simklData?.imdbVotes,
      countryOfOrigin,
      ageRating,
      ageRatingGuide,
      budget: movie.budget ? BigInt(movie.budget.replace(/[^0-9]/g, "") || 0) : undefined,
      revenue: movie.boxOffice ? BigInt(movie.boxOffice.replace(/[^0-9]/g, "") || 0) : undefined,
    ...
    if (existing) {
      localMovie = await prisma.movie.update({ where: { id: existing.id }, data });
    } else {
      ...
      for (const c of movie.characters) {
        ...
        if (localCharId) {
          await prisma.mediaCharacter.upsert({
            ...
            update: { movieId: localMovieId, role: c.isFeatured ? "MAIN" : "SUPPORTING" },
            create: {
              mediaType: "MOVIE",
              mediaId: localMovieId,
          ...
          await prisma.mediaStaff.upsert({
            ...
            update: { movieId: localMovieId, customRole: c.typeName || c.peopleType },
            create: {
              mediaType: "MOVIE",
              mediaId: localMovieId,
              movieId: localMovieId,
    ...
    if (movie.companies?.studio || movie.companies?.production) {
      const companies = [...(movie.companies.studio || []), ...(movie.companies.production || [])];
  ...
  async upsertBook(book: GoogleBookPayload): Promise<{ id: number }> {
    ...
    const description = info.description
      ? info.description.replace(/<[^>]*>?/gm, " ").replace(/\s+/g, " ").trim()
      : undefined;
    ...
    const data: any = {
      ...
      genres: existing ? { set: bookGenreRecords } : { connect: bookGenreRecords },
      tags: existing ? { set: tagRecords } : { connect: tagRecords },
    ...
    for (const ar of ageRatings) {
      ...
      if (org === 1 && cat) {
        ...
        if (ar.rating_content_descriptions && ar.rating_content_descriptions.length > 0) {
          ageRatingGuide = ar.rating_content_descriptions.map((d) => d.description).join(", ");
        } else if (ar.synopsis) {
  ...
  async upsertGame(
    ...
    const existing = await prisma.game.findUnique({ where: { igdbId: game.id } });
    ...
    const images = {
      igdb: [coverImage, ...(artworks || []), ...(screenshots || [])].filter(Boolean) as string[],
    };
    ...
    if (!steamAppId && game.websites) {
      const steamWeb = game.websites.find((w) => w.url.includes("store.steampowered.com/app/"));
      if (steamWeb) {
    ...
    if (game.player_perspectives) {
      for (const p of game.player_perspectives) {
        if (p.name) rawTags.push({ name: p.name, category: "Player Perspective" });
      }
    }
    const tagRecords = await this.upsertTags(rawTags);
    ...
    if (game.involved_companies) {
      for (const comp of game.involved_companies) {
        if (comp.developer && comp.company?.name && !developers.includes(comp.company.name)) {
          developers.push(comp.company.name);
        }
        if (comp.publisher && comp.company?.name && !publishers.includes(comp.company.name)) {
    ...
    // Age ratings
    const { esrbRating, pegiRating, ageRating, ageRatingGuide } = this.mapIgdbAgeRatings(
      game.age_ratings
    );

    // Franchise
    const franchise =
      game.franchise?.name || game.franchises?.[0]?.name || game.collection?.name;
    ...
    if (steamData) {
      const cleanHtml = (str?: string) =>
        str ? str.replace(/<[^>]*>?/gm, " ").replace(/\s+/g, " ").trim() : undefined;
      requirements = {
        pc:
          steamData.pc_requirements && !Array.isArray(steamData.pc_requirements)
            ...
            : undefined,
        mac:
          steamData.mac_requirements && !Array.isArray(steamData.mac_requirements)
            ? {
            ...
            : undefined,
        linux:
          steamData.linux_requirements && !Array.isArray(steamData.linux_requirements)
            ? {
                minimum: cleanHtml(steamData.linux_requirements.minimum),
                recommended: cleanHtml(steamData.linux_requirements.recommended),
              }
            : undefined,
      };
    }
    ...
    if (steamData?.controller_support) {
      ...
      if (steamData.categories.some((c) => c.id === 28)) controllerSupport = "Full";
      else if (steamData.categories.some((c) => c.id === 18)) controllerSupport = "Partial";
    }
    ...
    const data: any = {
      ...
      releaseDateMonth,
      releaseDateDay,
      genres: existing ? { set: gameGenreRecords } : { connect: gameGenreRecords },
      tags: existing ? { set: tagRecords } : { connect: tagRecords },
      ...
      averageScore: game.total_rating || game.rating || game.aggregated_rating,
  ...
  /**
   * Upserts Music record with lyrics, tags, genres, and metadata using LOCAL IDs.
   */
  async upsertMusic(
    mb: MusicBrainzRecordingPayload,
    lyrics?: LrcLibLyricsPayload | null
  ): Promise<{ id: number }> {
    const existing = await prisma.music.findUnique({ where: { musicBrainzId: mb.id } });
    ...
    if (release?.date) {
      const parts = release.date.split("-").map(Number);
      if (parts[0] && !isNaN(parts[0])) releaseDateYear = parts[0];
      if (parts[1] && !isNaN(parts[1])) releaseDateMonth = parts[1];
      if (parts[2] && !isNaN(parts[2])) releaseDateDay = parts[2];
      const parsedD = new Date(release.date);
      if (!isNaN(parsedD.getTime())) releaseDate = parsedD;
    }
    ...
    const images = {
      musicBrainz: [mb.coverImageUrl].filter(Boolean) as string[],
    };
    ...
    const data: any = {
      ...
      bannerImage: mb.coverImageUrl,
      images,
      sources,
      lyrics: fullPlainLyrics,
      syncedLyrics: lyrics?.syncedLyrics,
      musicBrainzUpdatedAt: Math.floor(Date.now() / 1000),
    };

    let music: { id: number };
    if (existing) {
      music = await prisma.music.update({ where: { id: existing.id }, data });
    } else {
      music = await prisma.music.create({ data });
    }
