...
const RUSTFS_ENDPOINT =
  process.env.RUSTFS_ENDPOINT || "http://127.0.0.1:9000";
  ...
  try {
    ...
    if (err?.name === "NotFound" || err?.$metadata?.httpStatusCode === 404) {
      try {
        ...
        console.warn(`[RustFS] Warning creating bucket ${PUBLIC_BUCKET}:`, createErr);
      }
    } else {
      // If error is other (e.g. connection refused in offline testing), log softly
      console.warn(`[RustFS] S3 HeadBucket info for ${PUBLIC_BUCKET}:`, err?.message || err);
    }
  }
}
...
export async function getPublicAsset(
  key: string
): Promise<{ body: ReadableStream | any; contentType?: string; contentLength?: number } | null> {
  try {
