...
export default defineRoute({
  ...
  schema: {
    ...
    detail: {
      ...
      description:
        "Finds similar TV series based on title (20 pts, >= 60% match), genres (1 pt each), and characters (0.5 pts each) with a minimum score of 17 points.",
      tags: ["Media - TV"],
    },
  },
  ...
  async GET({ params, query, prisma, cache, cacheKeys }) {
    ...
    const cacheKey = cacheKeys.similar.tv(id)
    const cached = await cache.get<SimilarMediaItem[]>(cacheKey)
    if (cached) {
      return cached.slice(0, limit)
    }

    const source = await prisma.tv.findUnique({
      where: { id },
      select: {
        id: true,
        titlePrimary: true,
    ...
    const orConditions: Prisma.TvWhereInput[] = []
    ...
    const candidates = await prisma.tv.findMany({
      where: {
        id: { not: source.id },
        OR: orConditions,
      },
