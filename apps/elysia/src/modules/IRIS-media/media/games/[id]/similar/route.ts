...
export default defineRoute({
  cacheKeys: {
    similar: {
      games: (id: number) => `games:${id}:similar`,
    },
  },
  ...
  schema: {
    ...
    detail: {
      ...
      description:
        "Finds similar games based on title (20 pts, >= 60% match) and genres (1 pt each) with a minimum score of 17 points.",
      tags: ["Media - Games"],
    },
  },
  ...
  async GET({ params, query, prisma, cache, cacheKeys }) {
    ...
    const cacheKey = cacheKeys.similar.games(id);
