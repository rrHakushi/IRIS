...
export default defineRoute({
  cacheKeys: {
    similar: {
      manga: (id: number) => `manga:${id}:similar`,
    },
  },
  ...
  schema: {
    ...
    detail: {
      summary: "Get similar manga series",
      description:
        "Finds similar manga series based on title (20 pts, >= 60% match), genres (1 pt each), and characters (0.5 pts each) with a minimum score of 17 points.",
      tags: ["Media - Manga"],
    },
  },
  ...
  async GET({ params, query, prisma, cache, cacheKeys }) {
    ...
    const source = await prisma.manga.findUnique({
      where: { id },
      select: {
        id: true,
        titlePrimary: true,
    ...
    const orConditions: Prisma.MangaWhereInput[] = []
