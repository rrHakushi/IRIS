...
export const STOPWORDS = new Set([
  "the", "a", "an", "no", "to", "in", "of", "and", "or", "for", "with", "on", "at", "by", "is", "it",
]);
...
export function tokenSimilarity(s1: string, s2: string): number {
  const words1 = s1.split(/\s+/).filter((w) => w.length > 0 && !STOPWORDS.has(w));
  const words2 = s2.split(/\s+/).filter((w) => w.length > 0 && !STOPWORDS.has(w));
  ...
  const overlapMin = Math.min(set1.size, set2.size) > 0 ? common / Math.min(set1.size, set2.size) : 0;
...
export function computeTitleSimilarity(str1: string, str2: string): number {
  ...
  const baseSim = (base1.length >= 3 && base2.length >= 3)
...
export function isTitleMatch(
  ...
  const validSource = sourceTitles.filter((t): t is string => Boolean(t && t.trim().length > 0));
  const validCandidate = candidateTitles.filter((t): t is string => Boolean(t && t.trim().length > 0));
...
export function extractSearchKeywords(titles: (string | null | undefined)[]): string[] {
