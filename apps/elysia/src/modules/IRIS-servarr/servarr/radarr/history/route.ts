  ...
  GET: {
    schema: {
      query: t.Object({
        ...
        eventType: t.Optional(t.Numeric()),
      ...
      detail: {
        summary: "Get Radarr activity history",
        description:
          "Fetches audit log of grab, import, upgrade, and file deletion events.",
        tags: ["Servarr", "Radarr"],
      },
    },
    ...
    async handler({ query, session, prisma }: any) {
      ...
      try {
        const data = await res.adapter.getHistory(res.credentials, {
          page: query?.page || 1,
        ...
        return {
          ...
          records: [],
          totalRecords: 0,
          message: err.message || "Failed to fetch history from Radarr",
        }
      }
    },
  },
