...
/**
 * Action Handler: "profile.comment.reply"
 * Allows profile owner to reply to a user profile comment directly from an interactive notification.
 * Enforces 1 reply max per comment.
 */
const profileCommentReplyAction: NotificationActionHandler = async ({
  notification,
  payload,
  resolvedStatus,
  sessionUser,
  ...
  // 2. Only profile owner can reply
  if (comment.listOwnerId !== sessionUser.id) {
    return {
      success: false,
      error: "Only the profile owner can reply to comments on this profile.",
    }
  }
  ...
  if (comment.reply) {
    return {
      success: false,
      error:
        "This comment already has an owner reply. You can edit the existing reply in the profile comments section.",
    }
  }
  ...
  if (comment.authorId !== sessionUser.id) {
    try {
      ...
      const ownerName =
        ownerCustomization.displayName || sessionUser.username || "Profile owner"
      ...
      console.warn(
        `[profileCommentReplyAction] Failed to send notification to commentator:`,
        notifErr
      )
    }
  }
...
export default profileCommentReplyAction
